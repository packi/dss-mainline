...
bool VdcDb::lookupStates(const std::string &gtin, std::vector<DeviceStateSpec_t> *out) {
  std::vector<DeviceStateSpec_t> &states(*out);
  states.clear();

  std::vector<StateDesc> in;
  if (!lookupStates(gtin, &in, "")) {
    return false;
  }

  foreach (const StateDesc desc, in) {
...
bool VdcDb::lookupStates(const std::string &gtin, std::vector<StateDesc> *out, const std::string &langCode) {
  ...
  try {
    ...
    // no dynamic dependeny, unless db is missing or sql schema changes
...
bool VdcDb::lookupProperties(const std::string &gtin, std::vector<PropertyDesc> *out, const std::string &langCode) {
