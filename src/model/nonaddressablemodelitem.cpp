...
namespace dss {
  ...
  void NonAddressableModelItem::pushSensor(const callOrigin_t _origin, const SceneAccessCategory _category, const dsuid_t _sourceID, SensorType _sensorType, const double _sensorValueFloat, const std::string _token) {
    std::vector<boost::shared_ptr<AddressableModelItem> > items = splitIntoAddressableItems();
    foreach(boost::shared_ptr<AddressableModelItem> item, items) {
